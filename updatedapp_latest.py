...
def predict_next_close(new_data_1):
    ...
    model_5 = load_model('next_close_prediction_model_5min.h5')
    ...
    print(new_values, 'new_values_ssssssssssssssssssssssssssssssssssssssssssss')
    # Scale the new data
    ...
    #for 5 min
    predicted_scaled_change_5 = model_5.predict(new_input)

    # Inverse transform the predicted values to get percentage change
    predicted_change_5 = scaler.inverse_transform(predicted_scaled_change_5)[0][-1]

    # Fetch the last 'Close' price from your data
    last_close_price_5 = new_values_1[-1][-1]

    # Apply the predicted percentage change to get the predicted next 'Close' price
    predicted_close_5 = last_close_price_5 * (1 + predicted_change_5)

    return predicted_close, predicted_close_5
...
def get_xausd_news():
    ...
    if response.status_code == 200:
        ...
        if len(df)>0:
            ...
            # value_counts = filtered_df['Sentiment'].value_counts()
            filtered_df = df[df['sentiment'] != 'Neutral']

            return filtered_df
        else:
            df = pd.DataFrame([{'date':'No_news','title':'No_news','sentiment':'Neutral'}])
            return df


model_sentiment = BertForSequenceClassification.from_pretrained('yiyanghkust/finbert-tone',num_labels=3)
...
def sent(strr):
    nlp = pipeline("text-classification", model=model_sentiment, tokenizer=tokenizer)
    results = nlp(strr)
...
while True:
    ...
    if stock_data.index[-1] != last_time:
        ...
        if len(news_df)==0:	
           news_df = pd.DataFrame([{'date':'No_news','title':'No_news','sentiment':'Neutral'}])
           print(news_df)
        ...
        if len(stock_data) >= 30:
            # try:
            pred_value,pred_value_5min = predict_next_close(stock_data)
